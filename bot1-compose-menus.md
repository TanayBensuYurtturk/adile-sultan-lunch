...
2. **randomly composes one complete menu per menu type** (a random main + a random side), with the
   total price, approximate calories **read from the `lunch.calories` table**, and a link to the
...
Also read the precomputed calorie estimates (one row per dish):

```sql
SELECT dish, approx_calories
FROM `bruin-playground-bensu.lunch.calories`;
```
...
Calories are **not** on the site — they're precomputed into `lunch.calories`, which the bot reads
(see Step 2).
...
- `approx_calories` = `calories[main] + calories[side]`, looked up from the `lunch.calories` rows
  read in Step 1 (fall back to 300 for any dish not found). It's an estimate, so keep the `~`.
...
(Kalori bilgileri yaklaşıktır.)

⏰ If you don't vote by 11:30, the default menu (👨‍🍳 Chef's Choice) will be selected for you.
```
